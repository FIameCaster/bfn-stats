...
// For custom events
interface UpgradeMenuEventMap {
	"upgadd": CustomEvent<number>
	"upgremove": CustomEvent<number>
	"upgclose": CustomEvent<unknown>
}
interface UpgradeMenuElement extends HTMLDivElement { 
	addEventListener<K extends keyof UpgradeMenuEventMap>(type: K, listener: (this: UpgradeMenuElement, ev: UpgradeMenuEventMap[K]) => void): void;
	dispatchEvent<K extends keyof UpgradeMenuEventMap>(ev: UpgradeMenuEventMap[K]): boolean;
}
interface TempMenuEventMap {
	"tempchange": CustomEvent<number>
}
interface TempMenuElement extends HTMLDivElement { 
	addEventListener<K extends keyof TempMenuEventMap>(type: K, listener: (this: UpgradeMenuElement, ev: TempMenuEventMap[K]) => void): void;
...
const upgradeMenu = (() => {
	...
	const updatePoints = (newPoints: number) => {
		countText.data = `Upgrades (${currentPoints = newPoints}/7)`
		firstExpensiveUpg?.classList.remove('expensive')
	...
	for (let i = 0; i < 21; i++) {
		let type: HTMLSpanElement, name: Text, 
...
const tempUpgMenu = (() => {	
	const list = element('ul', { className: 'temp-list' })

	return {
		open(char: Character, el: TempMenuElement, selected: Set<number>, state: number[]) {
			const upgs = upgrades[(char.owner || char).id],
			controls: HTMLLIElement[] = []

			// Recreating all elements every time the menu is opened is more than fast enough, so cba optimising
			let i = 0
			for (const id of selected) {
				const temp = upgs[id][5]
				if (!temp) continue
				let stageText: Text
				...
				controls[i++] = element('li', { className: temp[1] ? 'range' : 'input-group' }, [
					...
					element('input', {
						...
						oninput() {
							state[id] = temp[1] ? 
								+(stageText.data = this.value) : 
								this.checked ? 0 : null
							el.dispatchEvent(new CustomEvent('tempchange', { detail: id }))
						}
					})
				])
			}
			...
			loadStyleSheet(() => {
				list.style.maxHeight = `${Math.min(innerHeight - el.getBoundingClientRect().top - 40, 532)}px`
				el.append(list)
			})
			
		},
		isClosed() { return !list.parentElement },
		close() { list.remove() }
	}

})()

// Closing the menus if the user clicks outside them
addEventListener('click', e => {
	if (!(<HTMLElement>e.target).matches('#upg *')) upgradeMenu.close()
	if (!(<HTMLElement>e.target).matches('#temp *')) tempUpgMenu.close()
})

// Make sure stylesheet is loaded before showing the menus
const loadStyleSheet = (() => {
	let initialized = false

	return (callback: () => any) => {
		if (initialized == (initialized = true)) return callback()
		const stylesheet = router.prefetchedStylesheets['upgradeMenu']
...
const upgradeText = [
	...
	"Heal 25 HP over 3 seconds upon earning a vanquish at less than 20% health.",
	"Decrease sprint exit delay from 0.33 to 0.05 seconds.",
	"Grant 25 health over 3 seconds upon earning a hero vanquish. Can grant overhealth.",
	// Defensive
	...
	// Kernel Corn
	"Add fixed dispersion to Husk Hop of 8.59° alternating between left and right.",
	...
	"Enemies entering Casting Shadows lose 7 ammo and have their charge time increased by 50%, punch speed halved and 10% heat added.",
	...
	"Enemies damaged by Fung Fu are knocked back.",
	...
	"Decrease zoom FOV to 25° and displaces camera to the left while aiming.",
	...
	"Double rockets fired per target by Can't Miss‑ile.",
	...
	"Double angle to achieve Starz Align to 180°.",
...
export { upgradeMenu, tempUpgMenu, UpgradeMenuType, TempUpgMenuType, UpgradeMenuElement, TempMenuElement }
