...
const abilityData = [
	...
	() => ["E.M.Peach",20,.8,.15,,[750,,,,,,[[5,1,2,75,.65,[10,1.45,1,2.2]]]],,,["Stun",1.4,.33],7.5],
...
class Ability {
  ...
  constructor(data: any[], id: number) {
		...
		if (data[5]) this.weapon = new stats.Weapon(data[5], 0)
		if (id == 71) this.weapon.ignoreGrav = true
		if (data[18]?.[7]) this.vehicle[7] = new stats.Weapon(data[18][7], 0)
  }
}

stats.setAbilities((index: number) => abilityIDs[index].map((id: number) => new Ability(abilityData[id](), id)))
