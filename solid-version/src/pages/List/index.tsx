...
import { createMemo, Accessor, Index, Signal } from 'solid-js'
...
const TableBody = ({ category, sort, ascending, team }: { category: Accessor<string>, sort: Accessor<number>, ascending: Accessor<boolean>, team: Accessor<string> }) => {
	...
	const tableData: [Signal<Character>, Signal<number[]>][] = Array.from({ length: 34 }, () => [createSignal(), createSignal()])

	const sortedTableData = createMemo(() => {
		const newData: [Signal<Character>, Signal<number[]>][] = []
		...
		<Index each={sortedTableData()}>{
			data => {
				...
				return <tr>
					...
					<Index each={stats()}>{
						(stat, i) => <td>{
							stat() == null ? '' : round(stat(), categoryData()[0][i]) + categoryData()[1][i]
						}</td>
					}</Index>
				</tr>
			}
